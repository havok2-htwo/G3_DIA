...
def _normalize_audio_data(audio_data: np.ndarray) -> np.ndarray:
    ...
    # Keep source amplitude intact. PCM decoders already return conventional
    # floating-point scale and peak-normalization would amplify quiet noise.
...
def _seekable_ffmpeg_input(audio_file: BinaryIO, filename: str) -> Iterator[str]:
    """Copy an upload to a closed named file that ffmpeg can seek in.

    MP4/M4A/MOV containers may keep their metadata behind the media payload.
    Feeding those files over ``pipe:0`` makes the demuxer report ``partial
    file``.  Copying in bounded chunks avoids a second complete in-memory byte
    string and provides a portable seekable path on Windows and Linux.
    """
    ...
    try:
        ...
        if temp_path:
            try:
                ...
                # Temp cleanup must not discard an otherwise valid result.
                pass
...
def load_audio_bytes(audio_bytes: bytes, filename: str, target_sample_rate: int = 16000) -> np.ndarray:
    """Compatibility helper for internal callers that already own bytes."""
